...
public class TestSort {
    ...
    //非递归实现
    public static void mergeDo(int[] array,int gap){
        //创建一个新数组 在新数组里进行合并
        ...
        while(start2 < array.length){
            ...
            while (start1 <= end1 && start2 <= end2){
                ...
                if(array[start1] <= array[start2]){
                    ...
                    tem1[top] = array[start1];
                    top++;
                    start1++;
                }else {
                    tem1[top] = array[start2];
                    top++;
                    start2++;

                }
            }
            //跳出上面循环时 两个归并段可能没遍历完 或者不够两个归并段
            while (start1 <= end1){
                tem1[top] = array[start1];
                top++;
                start1++;
    ...
    public static void mergeSort1(int[] array){
        ...
        long end1 = System.nanoTime();//纳秒
        System.out.println("归并非递归排序："+(end1-start1)+"纳秒");
    }
